    ...
    public static void load() {
        try {
            ...
            try {
                ...
                log.info("No override properties found" + e);
            }

            String network = "/config/network/";

            HashMap<String, Class> propertiesArray = new HashMap<>(4);

            propertiesArray.put("network", Config.class);
            propertiesArray.put("svstats", SvStatsConfig.class);
//            propertiesArray.put("commons", CommonsConfig.class);
            propertiesArray.put("database", DatabaseConfig.class);


            for (Entry<String, Class> entry: propertiesArray.entrySet()) {
                Properties property = PropertiesUtils.load(network + entry.getKey() + ".properties");
                PropertiesUtils.overrideProperty(property, myProps);
                ConfigurableProcessor.process(entry.getValue(), property);
            }
